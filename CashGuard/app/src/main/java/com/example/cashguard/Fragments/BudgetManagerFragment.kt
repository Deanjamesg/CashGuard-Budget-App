...
class BudgetManagerFragment : Fragment(), BudgetManagerAdapter.OnBudgetChangedListener {
    ...
    override fun onBudgetChanged() {
        val newTotal = budgetAdapter.calculateNewTotal()

        val formattedTotal = String.format(Locale.getDefault(), "%.0f", newTotal)
    ...
    private fun setUpClickListeners() {
        binding.submitButton.setOnClickListener {
            ...
            if (changedCategories.isNotEmpty()) {
                val totalBudget = binding.tvBudgetTotal.text.toString().toDoubleOrNull() ?: 0.0
